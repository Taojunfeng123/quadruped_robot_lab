...
# Pre-defined configs
##
from robot_lab.assets.magiclab import MAGICDOG_W_CFG  # isort: skip


@configclass
class MagicDogWActionsCfg(ActionsCfg):
...
@configclass
class MagicDogWRewardsCfg(RewardsCfg):
...
@configclass
class MagicDogWRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    actions: MagicDogWActionsCfg = MagicDogWActionsCfg()
    rewards: MagicDogWRewardsCfg = MagicDogWRewardsCfg()
    ...
    def __post_init__(self):
        ...
        # ------------------------------Sence------------------------------
        self.scene.robot = MAGICDOG_W_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        ...
        # ------------------------------Actions------------------------------
        # reduce action scale
        self.actions.joint_pos.scale = {".*_hip_joint": 1.0, "^(?!.*_hip_joint).*": 0.25}
        ...
        self.rewards.feet_gait.params["synced_feet_pair_names"] = (("FL_wheel", "RR_wheel"), ("FR_wheel", "RL_wheel"))
        self.rewards.upward.weight = 1.0

        # If the weight of rewards is 0, set rewards to None
        if self.__class__.__name__ == "MagicDogWRoughEnvCfg":
